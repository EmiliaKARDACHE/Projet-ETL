...
def extract_random_sample(file_path, sample_size=50000, random_state=42):
    """
    Extract: Lit un dataset et extrait un échantillon aléatoire
    """
...
def transform_data(df):
    ...
    """
    print("🔧 TRANSFORMATION: Focus sur ingredients et instructions")
    
    df_transformed = df.copy()
    
    # 1. Vérifier les colonnes importantes
    ...
    # 2. Nettoyer les données manquantes pour ingredients et instructions
    ...
    # Extraire les ingredients principaux (supprimer quantités et détails)
    import re
    def extract_main_ingredient(ingredient_text):
        # Supprimer les quantités (chiffres + unités)
    ...
    print("   📝 Traitement des instructions...")
    ...
    # Diviser en étapes (par numéros ou points)
    ...
    # Extraire les mots-clés de cuisson
    cooking_methods = ['bake', 'fry', 'boil', 'grill', 'roast', 'steam', 'sauté', 'simmer', 'mix', 'blend']
    for method in cooking_methods:
        df_transformed[f'uses_{method}'] = df_transformed['instructions_clean'].str.contains(method, case=False, na=False)
    
    # 5. Statistiques finales
    print(f"   ✅ Ingredients moyens par recette: {df_transformed['ingredients_count'].mean():.1f}")
    print(f"   ✅ Étapes moyennes par recette: {df_transformed['steps_count'].mean():.1f}")
    print(f"   ✅ Longueur moyenne instructions: {df_transformed['instructions_length'].mean():.0f} caractères")
    
    # 6. Supprimer les doublons basés sur title + ingredients
    df_transformed = df_transformed.drop_duplicates(subset=['title', 'ingredients_clean'])
    print(f"   ✅ Lignes finales après dédoublonnage: {len(df_transformed)}")
...
def main():
    ...
    """
    # Chemin spécifique pour le dataset Spoonacular
    ...
    print(f"🎯 Dataset sélectionné: {selected_dataset.name}")
    print("🎯 Focus sur: ingredients et instructions (recipe description)")
